...
if (isset($_POST['update_order'])) {
    ...
    $date = date("d.m.Y");
    $result = mysqli_query($conn, "SELECT sub_total FROM `orders` WHERE id='$order_update_id'") or die('query failed');
    $row = mysqli_fetch_assoc($result);
    $order_sub_total = $row['sub_total'];
    mysqli_query($conn, "UPDATE `confirm_order` SET payment_status = '$update_payment', date='$date', total_price='$order_sub_total' WHERE order_id = '$order_update_id'") or die('query failed');
...
<body>
    ...
    <?php
    if (isset($message)) {
        foreach ($message as $msg) {  // Changed $message variable inside foreach loop to $msg to avoid conflict
            echo '
